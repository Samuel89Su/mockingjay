...
module.exports = function (session) {
    ...
    RedisStore.prototype.get = function (sid, fn) {
        var store = this;
        var psid = generateKey(store.prefix, sid, 'data')
        if (!fn) fn = noop;
        debug('GET "%s"', sid);

        store.client.hgetall(psid, function (er, data) {
            if (er) return fn(er);
            if (!data) return fn();
            ...
            try {
                ...
                result = store.serializer.deserialize(data);
            } catch (er) {
                return fn(er);
            }
    ...
    RedisStore.prototype.set = function (sid, sess, fn) {
        var store = this;
        var key = generateKey(store.prefix, sid, 'data')
        var internalKey = generateKey(store.prefix, sid, 'internal')
        if (!fn) fn = noop;

        try {
            //   var jsess = store.serializer.stringify(sess);
            var jsess = store.serializer.serialize(sess)
        } catch (er) {
            return fn(er);
        }
        ...
        if (!store.disableTTL) {
            ...
            store.client.hmset(internalKey, {
                'SessionTimeout': ttl
            })
        ...
        store.client.hmset(key, jsess, function (er) {
            if (er) return fn(er);
            debug('HMSET complete');
    ...
    RedisStore.prototype.destroy = function (sid, fn) {
        ...
        if (Array.isArray(sid)) {
            ...
            sid.forEach(function (s) {
                multi.del(generateKey(prefix, s, 'data'));
                multi.del(generateKey(prefix, s, 'internal'));
            });
            multi.exec(fn);
        } else {
            var multi = this.client.multi();
            multi.del(generateKey(this.prefix, sid, 'data'));
            multi.del(generateKey(this.prefix, sid, 'internal'));
            multi.exec(fn);
        }
    };
    ...
    RedisStore.prototype.ids = function (fn) {
        ...
        allKeys(store, function (err, keys) {
            ...
            keys = keys.map(function (key) {
                return key.substr(prefixLength, key.lastIndexOf('_')-prefixLength);
            });
            return fn(null, keys);
        });
    };
            ...
            store.client.mget(keys, function (err, sessions) {
                ...
                try {
                    result = sessions.map(function (data, index) {
                        ...
                        data = store.serializer.deserialize(data);
                        data.id = keys[index].substr(prefixLength, keys[index].lastIndexOf('_')-prefixLength);
                        return data;
                    });
                } catch (e) {
                    err = e;
...
function generateKey(prefix, sid, postfix) {
    return `${prefix}${sid}_${postfix}`
}